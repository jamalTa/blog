...
class UsersController extends AbstractController
{
    ...
    public function new(Request $request, EntityManagerInterface $entityManager): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if($avatar)
            {
                $avatar_file_name = uniqid(). '.'. $avatar->guessExtension();
                //enregistrer le fichier dans le dossier image
                $avatar->move($this->getParameter('upload_dir'), $avatar_file_name);
                //une fois enregistrer je set l object article
                $user->setAvatar($avatar_file_name);
             
            }else{
                $user->setAvatar('imgDefault.jpg');
            }
